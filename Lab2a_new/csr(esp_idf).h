...
#ifndef _ESP_IDF_CSR_H_
#define _ESP_IDF_CSR_H_

#include <stdint.h>
#include <bits/stdint-uintn.h>

#define RV_READ_CONST_CSR(reg) ({ uint32_t __tmp; \
  asm ("csrr %0, " _CSR_STRINGIFY(reg) : "=r"(__tmp)); __tmp; })

#define RV_READ_CSR(reg) ({ uint32_t __tmp; \
  asm volatile ("csrr %0, " _CSR_STRINGIFY(reg) : "=r"(__tmp)); __tmp; })

#define RV_WRITE_CSR(reg, val) ({ asm volatile("csrw " _CSR_STRINGIFY(reg) ", %0" ::"rK"(val)); })

#define RV_SWAP_CSR(reg, val) ({ uint32_t __tmp; \
...
#define RV_SET_CSR(reg, bit) ({ uint32_t __tmp; \
...
#define RV_CLEAR_CSR(reg, bit) ({ uint32_t __tmp; \
